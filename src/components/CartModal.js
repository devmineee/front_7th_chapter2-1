...
const CartWithItems = (cart, selectedIds) => {
  const selectedItems = cart.filter((item) => selectedIds.includes(item.productId));
  const selectedTotal = selectedItems.reduce((sum, item) => sum + Number(item.lprice) * item.quantity, 0);
  const totalAmount = cart.reduce((sum, item) => sum + Number(item.lprice) * item.quantity, 0);
        ...
        <div class="flex flex-col max-h-[calc(90vh-120px)]">
          ...
          <div class="p-4 border-b border-gray-200 bg-gray-50">
            <label class="flex items-center text-sm text-gray-700">
              <input type="checkbox" id="cart-modal-select-all-checkbox" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 mr-2" ${selectedIds.length === cart.length ? "checked" : ""}>
              전체선택 (${cart.length}개)
            </label>
          </div>
          <!-- 아이템 목록 -->
          <div class="flex-1 overflow-y-auto">
            <div class="p-4 space-y-4">
              ${cart.map((item) => CartItem(item, selectedIds.includes(item.productId))).join("")}
            </div>
          </div>
        </div>
        <!-- 하단 액션 -->
...
export const CartModal = ({ cart = [], selectedIds = [] }) => {
  ...
  return CartWithItems(cart, selectedIds);
};
